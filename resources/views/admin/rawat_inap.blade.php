...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
  ...
  <div class="flex gap-2">
    <a href="{{ route('admin.rawat_inap', ['status' => 'Aktif']) }}" class="px-4 py-2 text-sm font-medium rounded-xl border transition {{ request('status', 'Aktif') == 'Aktif' ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50' }}">Sedang Dirawat</a>
    <a href="{{ route('admin.rawat_inap', ['status' => 'Selesai']) }}" class="px-4 py-2 text-sm font-medium rounded-xl border transition {{ request('status') == 'Selesai' ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50' }}">Selesai / Pulang</a>
  </div>

  <!-- Check-in Button -->
  <button onclick="openModal('checkInModal')" class="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-medium transition shadow-sm flex items-center justify-center gap-2 text-sm">
      ...
      <tbody class="divide-y divide-gray-100">
        @forelse ($rawat_inaps as $ri)
          <tr class="hover:bg-blue-50/50 transition group">
            ...
            <td class="px-6 py-4">
              <span class="block font-semibold {{ $ri->jenis_penjamin == 'Umum' ? 'text-gray-800' : 'text-emerald-700' }}">{{ $ri->jenis_penjamin }}</span>
              <span class="text-xs text-gray-500">DPJP: dr. {{ $ri->dokter->nama }}</span>
            </td>
            <td class="px-6 py-4 text-center">
              @if($ri->status === 'Aktif')
                <span class="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-bold animate-pulse"><i class="fas fa-bed mr-1"></i> Dirawat</span>
              @else
                <span class="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold"><i class="fas fa-check-circle mr-1"></i> Selesai</span><br>
                <span class="text-[10px] text-gray-400 mt-1 block">Tgl Keluar: {{ $ri->tgl_keluar->format('d/m/Y') }}</span>
              @endif
            </td>
            <td class="px-6 py-4 text-right space-x-1">
              @if($ri->status === 'Aktif')
                <button onclick="openCheckoutModal({{ $ri->id }}, '{{ $ri->pasien->nama }}')" class="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-xs font-bold transition shadow-sm">
                  <i class="fas fa-sign-out-alt mr-1"></i> Pulang
                </button>
              @endif
              @if($ri->billing)
                <a href="{{ route('admin.billing.show', $ri->billing->id) }}" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-xs font-bold transition shadow-sm border border-gray-200">
      ...
      <div class="p-6 space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select name="pasien_id" required class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 text-sm">
          ...
          <div>
            ...
            <select name="kamar_id" required class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 text-sm">
              <option value="">-- Pilih Kamar Tersedia --</option>
              @foreach($kamarsTersedia as $k)
                <option value="{{ $k->id }}">{{ $k->nama_kamar }} ({{ $k->kelas }}) - Rp {{ number_format($k->tarif_per_malam, 0, ',', '.') }}/malam</option>
              @endforeach
            </select>
            @if($kamarsTersedia->isEmpty())
                <p class="text-xs text-red-500 mt-1"><i class="fas fa-exclamation-triangle"></i> Tidak ada kamar tersedia.</p>
            @endif
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select name="dokter_id" required class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 text-sm">
          ...
          <div>
            ...
            <input type="datetime-local" name="tgl_masuk" value="{{ now()->format('Y-m-d\TH:i') }}" required class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 text-sm">
          </div>
        </div>
        ...
        <div class="p-4 bg-blue-50 border border-blue-100 rounded-xl space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select name="jenis_penjamin" id="jenis_penjamin" onchange="toggleSep()" required class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 text-sm">
            ...
            <div id="no_sep_container" class="hidden">
              ...
              <input type="text" name="no_sep" id="no_sep" placeholder="Wajib untuk klaim" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 text-sm">
            </div>
          </div>
        </div>
      </div>
      <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
        <button type="button" onclick="closeModal('checkInModal')" class="px-4 py-2 text-gray-600 hover:bg-gray-200 rounded-xl font-medium transition text-sm">Batal</button>
        <button type="submit" class="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-medium transition shadow-sm text-sm" @if($kamarsTersedia->isEmpty()) disabled @endif>Proses Check-In</button>
      </div>
    </form>
  </div>
</div>
    ...
    <form id="checkOutForm" method="POST">
      ...
      <div class="p-6 space-y-4">
        ...
        <div>
          ...
          <input type="datetime-local" name="tgl_keluar" value="{{ now()->format('Y-m-d\TH:i') }}" required class="w-full border-gray-300 rounded-xl focus:ring-amber-500 focus:border-amber-500 text-sm">
        </div>
        <div>
          <label class="block text-sm font-semibold text-gray-700 mb-1">Catatan Kepulangan (Opsional)</label>
          <textarea name="catatan_keluar" rows="2" class="w-full border-gray-300 rounded-xl focus:ring-amber-500 focus:border-amber-500 text-sm" placeholder="Misal: Sembuh, Rujuk ke RSUD..."></textarea>
        </div>
        <p class="text-xs text-gray-500 mt-2"><i class="fas fa-info-circle"></i> Biaya rawat inap akan otomatis dikalkulasi ke tagihan kasir setelah proses check-out.</p>
      </div>
      <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
        <button type="button" onclick="closeModal('checkOutModal')" class="px-4 py-2 text-gray-600 hover:bg-gray-200 rounded-xl font-medium transition text-sm">Batal</button>
        <button type="submit" class="px-5 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-xl font-medium transition shadow-sm text-sm">Selesaikan Perawatan</button>
      </div>
    </form>
  </div>
</div>
